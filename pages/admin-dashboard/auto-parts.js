...
const openAutoPartModal = () => {
  setAddModal(true);
}
const editItem = (autoPart) => {
   setSelectedItem(autoPart)
    setEditModal(true);
}
const handleDelete = (slug) => {
  A.Swal.fire({
    ...
    if (result.value) {
     dispatch(A.deleteItem({url: '/api/auto-parts/', slug: slug}))
...
A.useEffect(() => {
  ...
  return () => {
    ...
    return(
        ...
        <A.Layout>
            ...
            <button  className="w3-bar-item w3-button w3-blue w3-right" onClick={openAutoPartModal}>Add New</button>
            </div>
      
       <AddItem
        addModal={addModal} setAddModal={setAddModal} 
          schema={schema} inputLabel={'Auto part'} inputName={'type'}
          title={'Add new auto part'} path={'/api/auto-parts'}
        />
       <EditItem
        editModal={editModal} setEditModal={setEditModal} item={selectedItem}
        schema={schema} inputLabel={'Auto part'} inputName={'type'}
          title={'Update auto part'} path={'/api/auto-parts/'} defaultValue={selectedItem.type}
        />
    <div className="card">
  <div className="card-header w3-blue">
    <h5 className="text-center ">Auto parts</h5>
  </div>
  ...
  {types && types.map(autoPart => (
    <tr key={autoPart.id}>
      ...
      onClick={() => handleDelete(autoPart.slug)}>delete</button></td>
    </tr>
  ))}
...
export default A.AuthRoute(AutoParts)

export const getServerSideProps = A.wrapper.getServerSideProps(
  async ({store, req}) => {
    const cookie = new A.Cookies(req.headers.cookie);
  await store.dispatch(A.loadItems({url: '/api/auto-parts', cookie: cookie.get('token')}))
  }
)