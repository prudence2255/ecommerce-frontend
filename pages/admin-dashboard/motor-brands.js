...
const openMotorBrandModal = () => {
  setAddModal(true);
}
const editItem = (motorBrand) => {
   setSelectedItem(motorBrand)
    setEditModal(true);
}
const handleDelete = (slug) => {
  A.Swal.fire({
    ...
    if (result.value) {
     dispatch(A.deleteItem({url: '/api/motor-brands/', slug: slug}))
      .then(A.unwrapResult).then(() => {
        A.Swal.fire(
          'Deleted!',
          'motor brand has been deleted.',
          'success'
        )
      }).catch((e) => e)
...
A.useEffect(() => {
  ...
  return () => {
    ...
    return(
        ...
        <A.Layout>
            ...
            <button  className="w3-bar-item w3-button w3-blue w3-right" onClick={openMotorBrandModal}>Add New</button>
            </div>
      
       <AddItem
        addModal={addModal} setAddModal={setAddModal} 
          schema={schema} inputLabel={'Brand'} inputName={'brand'}
          title={'Add new motor brand'} path={'/api/motor-brands'}
        />
       <EditItem
        editModal={editModal} setEditModal={setEditModal} item={selectedItem}
        schema={schema} inputLabel={'Brand'} inputName={'brand'}
          title={'Update motor brand'} path={'/api/motor-brands/'} defaultValue={selectedItem.brand}
        />
    <div className="card">
  <div className="card-header w3-blue">
    <h5 className="text-center ">motor Brands</h5>
  </div>
  ...
  <tbody>
  {brands && brands.map(motorBrand => (
    <tr key={motorBrand.id}>
      <td>{motorBrand.brand}</td>
      <td><button className="w3-btn w3-yellow" onClick={
        () => editItem(motorBrand)
      }>edit</button></td>
      <td><button className="w3-btn w3-red" 
      onClick={() => handleDelete(motorBrand.slug)}>delete</button></td>
    </tr>
  ))}
...
export default A.AuthRoute(MotorBrands)
export const getServerSideProps = A.wrapper.getServerSideProps(
  async ({store, req}) => {
    const cookie = new A.Cookies(req.headers.cookie);
  await store.dispatch(A.loadItems({url: '/api/motor-brands', cookie: cookie.get('token')}))
  }
)