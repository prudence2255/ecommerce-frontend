...
function ComputerAccessoryTypes() {
...
const editItem = (computerAccessoryType) => {
   setSelectedItem(computerAccessoryType)
    setEditModal(true);
}
const handleDelete = (slug) => {
  A.Swal.fire({
    ...
    if (result.value) {
     dispatch(A.deleteItem({url: '/api/computer-accessories/', slug: slug}))
...
A.useEffect(() => {
  ...
  return () => {
    ...
    return(
        ...
        <A.Layout>
            ...
            <button  className="w3-bar-item w3-button w3-blue w3-right" onClick={openComputerAccessoryTypeModal}>Add New</button>
            </div>
      
       <AddItem
        addModal={addModal} setAddModal={setAddModal} 
          schema={schema} inputLabel={'Device type'} inputName={'type'}
          title={'Add new computer accessory type'} path={'/api/computer-accessories'}
        />
       <EditItem
        editModal={editModal} setEditModal={setEditModal} item={selectedItem}
        schema={schema} inputLabel={'Accessory type'} inputName={'type'}
          title={'Update computer accessory type'} path={'/api/computer-accessories/'} defaultValue={selectedItem.type}
        />
    <div className="card">
  ...
  <tbody>
  {types && types.map(computerAccessoryType => (
    <tr key={computerAccessoryType.id}>
      <td>{computerAccessoryType.type}</td>
      <td><button className="w3-btn w3-yellow" onClick={
        () => editItem(computerAccessoryType)
      }>edit</button></td>
      <td><button className="w3-btn w3-red" 
      onClick={() => handleDelete(computerAccessoryType.slug)}>delete</button></td>
    </tr>
  ))}
...
export default A.AuthRoute(ComputerAccessoryTypes)
export const getServerSideProps = A.wrapper.getServerSideProps(
  async ({store, req}) => {
    const cookie = new A.Cookies(req.headers.cookie);
  await store.dispatch(A.loadItems({url: '/api/computer-accessories', cookie: cookie.get('token')}))
  }
)