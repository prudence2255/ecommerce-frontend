...
const openParentServiceModal = () => {
  setAddModal(true);
}
const editItem = (parentService) => {
   setSelectedItem(parentService)
    setEditModal(true);
}
const handleDelete = (slug) => {
  A.Swal.fire({
    ...
    if (result.value) {
     dispatch(A.deleteItem({url: '/api/parent-services/', slug: slug}))
      .then(A.unwrapResult).then(() => {
        A.Swal.fire(
          'Deleted!',
          'parentService has been deleted.',
          'success'
        )
      }).catch((e) => e)
...
A.useEffect(() => {
  ...
  return () => {
    ...
    return(
        ...
        <A.Layout>
            ...
            <button  className="w3-bar-item w3-button w3-blue w3-right" onClick={openParentServiceModal}>Add New</button>
            </div>
      
       <AddItem
        addModal={addModal} setAddModal={setAddModal} 
          schema={schema} inputLabel={'parent'} inputName={'parent'}
          title={'Add new Service parent'} path={'/api/parent-services'}
        />
       <EditItem
        editModal={editModal} setEditModal={setEditModal} item={selectedItem}
        schema={schema} inputLabel={'parent'} inputName={'parent'}
          title={'Update Service parent'} path={'/api/parent-services/'} defaultValue={selectedItem.parent}
        />
    <div className="card">
  <div className="card-header w3-blue">
    <h5 className="text-center ">Service Parents</h5>
  </div>
  ...
  <tbody>
  {parents && parents.map(parentService => (
    <tr key={parentService.id}>
      <td>{parentService.parent}</td>
      <td><button className="w3-btn w3-yellow" onClick={
        () => editItem(parentService)
      }>edit</button></td>
      <td><button className="w3-btn w3-red" 
      onClick={() => handleDelete(parentService.slug)}>delete</button></td>
    </tr>
  ))}
...
export const getServerSideProps = A.wrapper.getServerSideProps(
  ...
  await store.dispatch(A.loadItems({url: '/api/parent-services', cookie: cookie.get('token')}))
  }
)